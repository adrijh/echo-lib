...
class SipService:
    ...
    async def create_room_for_call(self, event: events.StartSessionRequest, ai_number: str) -> api.Room:
        ...
        return await self.lk_api.room.update_room_metadata(
            api.UpdateRoomMetadataRequest(room=room_name, metadata=meta_payload)
        )


    async def initiate_sip_call(self, room: api.Room, event: events.StartSessionRequest, ai_number: str) -> None:
        log.info(
            f"Initiating call to  {event.participant_name} - {event.phone_number} from {ai_number} in room {room.name}"
        )
        ...
        try:
            ...
            await self._start_recording(room)
            ...
            if ("Busy Here" in str(e) or
                ...
                log.warning(
                    ...
                    f"Code: 486/Busy. Room {room.name} will be closed."
                )
                await self.cleanup_room(room.name)
                return

            log.error(f"Twirp Error in call {room.name}: {e}", exc_info=True)
            await self.cleanup_room(room.name)
            raise e

        except Exception as e:
            log.error(f"Unexpected Error in call {room.name}: {e}", exc_info=True)
            try:
                await self.cleanup_room(room.name)
    ...
    @classmethod
    async def run_background_call(cls, room: api.Room, event: events.StartSessionRequest, ai_number: str) -> None:
        service = cls()
        try:
            await service.initiate_sip_call(room, event, ai_number)
        except Exception as e:
            log.error(f"Error in background task for {room.name}: {e}")
        finally:
            await service.close()

    async def _start_recording(self, room: api.Room) -> None:
        egress_request = RoomCompositeEgressRequest(
            ...
            file_outputs=[
                EncodedFileOutput(
                    filepath=(f"recordings/{room.sid}/recording.ogg"),
