...
class TwilioService:
    ...
    def _create_attachment(self, content: str, filename: str = "transcripcion_original.txt") -> Attachment:
        ...
        return Attachment(
            FileContent(encoded_file), FileName(filename), FileType("text/plain"), Disposition("attachment")
        )

    def send_summary_email(
        self, subject: str, email_to: str, content_markdown: str, attachment_content: str | None = None
    ) -> None:
        logger.info(f"Sending summary email to {email_to}...")

        full_html = self._generate_html_content(content_markdown)

        message = Mail(from_email=cfg.SENDGRID_MAIL_FROM, to_emails=email_to, subject=subject, html_content=full_html)
